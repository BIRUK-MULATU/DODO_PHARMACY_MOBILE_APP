...
///
/// Shared by both apps: the user app opens this from the e-book collection
/// (where "unlock" means the real pay-and-approve flow) and the admin app
/// opens it to preview a book a specific user purchased (where an admin
/// session already comes back `unlocked: true` from the backend, so the
/// unlock button never actually renders — [onUnlock] exists at all only so
/// this screen doesn't need to import either app's own route table).
class EBookReaderScreen extends StatefulWidget {
  const EBookReaderScreen({super.key, required this.book, required this.onUnlock});

  final EBook book;

  /// Called when the reader's "Unlock" button is tapped. The user app wires
  /// this to its real payment flow; the admin app passes a no-op.
  final Future<void> Function(EBook book) onUnlock;
